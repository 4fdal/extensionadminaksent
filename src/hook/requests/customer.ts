    ...
    return {
        ...
        async reqAllCustomers(resync: boolean = false) {
            try {
                ...
                const TOTAL_FETCH_LIMIT = 5000; // Use a large enough limit to fetch all enrichment data
                ...
                if (pref.value && !resync) {
                    ...
                    dtProfileCustomer = await httpGetProfileCustomer({
                        length: TOTAL_FETCH_LIMIT,
                    });
                    await Preferences.set({
                        key: "dtProfileCustomer",
                ...
                if (pref.value && !resync) {
                    ...
                    dtUnpaidCustomer = await httpGetUnpaidCustomer({
                        length: TOTAL_FETCH_LIMIT,
                    });
                    await Preferences.set({
                        key: "dtUnpaidCustomer",
                ...
                if (pref.value && !resync) {
                    ...
                    dtPaidCustomer = await httpGetPaidCustomer({
                        length: TOTAL_FETCH_LIMIT,
                    });
                    await Preferences.set({
                        key: "dtPaidCustomer",
